...
public class CommBMessage {
	...
	private void decodeBDS50(){
		...
		gorundSpeed = ((short) (((message[3]<<8&0xFF00 | message[4]&0xFF)>>6)&0x3FF)) * 2;
		...
		trueAirspeed = ((short) ((message[5]<<8&0xFF00 | message[6]&0xFF)&0x3FF)) * 2;
		short test = (short) (message[5]<<8&0xFF00 | message[6]&0xFF);

		// here reasonableness test 80155b347ffccf
	...
	private void decodeBDS60(){
		...
		if (0<magneticHeading && magneticHeading<360 && LastDatas.hasLastData(icao) && LastDatas.getAircraftData(icao).hasAlt()){
			double lastAlt = LastDatas.getAircraftData(icao).getAlt(); // meters 
			double p0 = 101325; // Pa
	        ...
	        if (Math.abs(m - mach) <= 0.15 && Math.abs(barometricAltRate - inertialVertVel) <= 1000){
	        	hasBDS60 = true;
	        }
		}
	}
